...
func newAccount(db *gorm.DB, opts ...gen.DOOption) account {
	...
	_account.CustomerID = field.NewUint(tableName, "customer_id")
	_account.Name = field.NewString(tableName, "name")
	_account.No = field.NewString(tableName, "no")
	_account.Balance = field.NewFloat64(tableName, "balance")
	_account.Customer = accountBelongsToCustomer{
		...
		Accounts: struct {
			...
			RelationField: field.NewRelation("Customer.Accounts", "model.AccountList"),
			Customer: struct {
				field.RelationField
			}{
				RelationField: field.NewRelation("Customer.Accounts.Customer", "model.Customer"),
			},
		},
	}
...
type account struct {
	...
	Customer   accountBelongsToCustomer
...
func (a *account) updateTableName(table string) *account {
	...
	a.CustomerID = field.NewUint(table, "customer_id")
...
type accountBelongsToCustomer struct {
	db *gorm.DB
...
func (a accountBelongsToCustomer) Where(conds ...field.Expr) *accountBelongsToCustomer {
	if len(conds) == 0 {
		return &a
	}
...
func (a accountBelongsToCustomer) WithContext(ctx context.Context) *accountBelongsToCustomer {
	a.db = a.db.WithContext(ctx)
	return &a
}

func (a accountBelongsToCustomer) Model(m *model.Account) *accountBelongsToCustomerTx {
	return &accountBelongsToCustomerTx{a.db.Model(m).Association(a.Name())}
}

type accountBelongsToCustomerTx struct{ tx *gorm.Association }

func (a accountBelongsToCustomerTx) Find() (result *model.Customer, err error) {
	return result, a.tx.Find(&result)
}

func (a accountBelongsToCustomerTx) Append(values ...*model.Customer) (err error) {
...
func (a accountBelongsToCustomerTx) Replace(values ...*model.Customer) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a accountBelongsToCustomerTx) Delete(values ...*model.Customer) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a accountBelongsToCustomerTx) Clear() error {
	return a.tx.Clear()
}

func (a accountBelongsToCustomerTx) Count() int64 {
	return a.tx.Count()
}
