...
func newCustomer(db *gorm.DB, opts ...gen.DOOption) customer {
	...
	_customer.Password = field.NewString(tableName, "password")
	_customer.Accounts = customerHasManyAccounts{
		db: db.Session(&gorm.Session{}),

		RelationField: field.NewRelation("Accounts", "model.AccountList"),
		Customer: struct {
			field.RelationField
			Accounts struct {
				field.RelationField
			...
			RelationField: field.NewRelation("Accounts.Customer", "model.Customer"),
			Accounts: struct {
				field.RelationField
			}{
				RelationField: field.NewRelation("Accounts.Customer.Accounts", "model.AccountList"),
			},
		},
	}
...
type customer struct {
	...
	Accounts  customerHasManyAccounts
...
type customerHasManyAccounts struct {
	db *gorm.DB
...
func (a customerHasManyAccounts) Where(conds ...field.Expr) *customerHasManyAccounts {
	if len(conds) == 0 {
		return &a
	}
...
func (a customerHasManyAccounts) WithContext(ctx context.Context) *customerHasManyAccounts {
	a.db = a.db.WithContext(ctx)
	return &a
}

func (a customerHasManyAccounts) Model(m *model.Customer) *customerHasManyAccountsTx {
	return &customerHasManyAccountsTx{a.db.Model(m).Association(a.Name())}
}

type customerHasManyAccountsTx struct{ tx *gorm.Association }

func (a customerHasManyAccountsTx) Find() (result []*model.AccountList, err error) {
	return result, a.tx.Find(&result)
}

func (a customerHasManyAccountsTx) Append(values ...*model.AccountList) (err error) {
...
func (a customerHasManyAccountsTx) Replace(values ...*model.AccountList) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a customerHasManyAccountsTx) Delete(values ...*model.AccountList) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a customerHasManyAccountsTx) Clear() error {
	return a.tx.Clear()
}

func (a customerHasManyAccountsTx) Count() int64 {
	return a.tx.Count()
}
